  ...
  class Battle(renpy.Displayable):
    ...
    def loadBattle(self, number):
      ...
      if number == 15:
        ...
        self.enemies = [Crab(), Crab()]
      if number ==16:
        self.scenery.extend(riverGrnd)
        self.enemies = [Wight(), Crab(), Wolf()]
      if number == 17:
        self.scenery.extend(riverGrnd)
        self.enemies = [Baron()]
      self.initialize()
...
######################### BEGIN ENEMY DECLARATION #########################################
  ...
  class Baron(Fighter):
      def __init__(self):
        ...
        Fighter.__init__(self, animLib, position = (0,0), centerOffset = 0, attackCD = 160, attackDamage = 30, maxHP = 600)
        self.eventTimer = 0
